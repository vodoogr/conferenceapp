...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'GET') {
      const rows = await sql('SELECT * FROM transcriptions WHERE meeting_id = $1', [meetingId]);
      if (rows.length === 0) return errorResponse(res, new Error('No transcription yet'), 404);
